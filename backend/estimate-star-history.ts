...
import { estimateStarHistoryProcessing } from "./services/scraping-services/batched-star-history";
...
async function main() {
  try {
    ...
    if (repoCount === 0) {
      console.log(
        chalk.yellow(
          "No repositories found. Run the scraper first to get repository data.",
        ),
      );
      process.exit(0);
    }
    ...
    console.log(
      `   Estimated total API calls: ${estimation.estimatedApiCalls}`,
    );
    ...
    if (estimation.totalBatches > 1) {
      ...
      console.log(
        chalk.yellow(
          `   with 1-hour delays between batches to respect GitHub API limits.`,
        ),
      );
    }
  } catch (error) {
