...
import { prepTrendingData, saveTrendingData, saveStarHistoryBatch, } from "./services/repo-data.js";
dotenv.config();
...
export async function runScrapeJob() {
    ...
    try {
        ...
        console.log(`⭐ Starting star history collection for ${repoNames.length} repos...`);
        const starHistoryResult = await saveStarHistoryBatch(repoNames);
        console.log(`⭐ Star history completed: ${starHistoryResult.successful} successful, ${starHistoryResult.failed} failed, ${starHistoryResult.skipped} skipped`);
        process.exitCode = 0;
    }
    catch (err) {
        ...
        console.log(`[${new Date().toISOString()}] ⏹ Scrape job completed with exit code ${process.exitCode}`);
    }
}
