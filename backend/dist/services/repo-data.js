...
export async function prepTrendingData() {
    ...
    const results = await Promise.allSettled(trendingRepoNames.map((repoName) => processOneRepo(repoName, today)));
    const repos = results
...
export async function saveTrendingData(repos) {
    if (!repos.length)
        return; // guard against empty ops
...
export async function saveStarHistoryBatch(repoNames) {
    ...
    const repoMap = new Map(existingRepos.map((repo) => [repo.fullName, repo._id]));
    ...
    if (missingRepos.length > 0) {
        console.warn(`Skipping ${missingRepos.length} repos not found in DB:`, missingRepos);
    }
    ...
    for (let i = 0; i < validRepoNames.length; i++) {
        ...
        try {
            console.log(`[${i + 1}/${validRepoNames.length}] Processing ${repoName}...`);
    ...
    if (successfulResults.length > 0) {
        await StarHistory.insertMany(successfulResults.map(({ repoId, history }) => ({
            repoId,
            history,
        })));
        console.log(`Successfully saved star history for ${successfulResults.length} repos`);
    }
    // Log failures
    const failures = results.filter((result) => result.status === "rejected");
    if (failures.length > 0) {
        console.error(`Failed to process ${failures.length} repos:`, failures.map((f) => f.reason.message));
    }
    return {
...
async function fetchLanguages(url) {
    ...
    if (!res.ok) {
        throw new Error(`fetchLanguages(${url}) failed: ${res.status} ${res.statusText}`);
    }
    return res.json();
}
/**
...
function transformRepo(rawdata, languages, today) {
    // API Raw data
    const { full_name, name, owner: { login: owner }, description, html_url, license, created_at, // "2015-05-26T20:55:45Z",
...
//# sourceMappingURL=repo-data.js.map