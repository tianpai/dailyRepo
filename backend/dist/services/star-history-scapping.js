...
function compareRateLimits(before, after) {
    ...
    Object.keys(before.resources).forEach((resourceType) => {
        ...
        const remainingDifference = afterResource.remaining - beforeResource.remaining;
        if (usedDifference > 0 || remainingDifference !== 0) {
            console.log(`\n${resourceType.toUpperCase()}:`);
            console.log(`  Used: ${beforeResource.used} → ${afterResource.used} (+${usedDifference})`);
            console.log(`  Remaining: ${beforeResource.remaining} → ${afterResource.remaining} (${remainingDifference})`);
...
async function getRateLimit() {
    try {
        ...
        console.log("Full Rate Limit Data:", JSON.stringify(response.data, null, 2));
...
//# sourceMappingURL=star-history-scapping.js.map