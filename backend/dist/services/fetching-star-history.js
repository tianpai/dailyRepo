 ...
 */
export async function getRepoStarRecords(repo, token = process.env.GITHUB_TOKEN, maxRequestAmount = 60) {
    ...
    if (pageCount < maxRequestAmount) {
        ...
        // sample pages evenly
        requestPages = range(1, maxRequestAmount).map((i) => Math.max(1, Math.round((i * pageCount) / maxRequestAmount) - 1));
        if (!requestPages.includes(1)) {
            requestPages[0] = 1;
        }
    }
    // 4) fetch all those pages in parallel
    const responses = await Promise.all(requestPages.map((pg) => getRepoStargazers(repo, token, pg)));
...
//# sourceMappingURL=fetching-star-history.js.map